@extends('templates.middleware', ['titulo'=>"Alterar Livro"])

@section('conteudo')

<form action="{{ route('livros.update', $data->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col" >
        ...
        <div class="col">
            ...
            <label class="input-group-text bg-success text-white" for="inputGroupSelect01" >Generos</label>
                <select name="genero_id" class="form-control {{ $errors->has('genero_id') ? 'is-invalid' : ''}}">
                    ...
                    <option value="{{$item->id}}" @if($item->id == $data['genero_id']) selected="true" @endif>
                        {{ $item->nome }}
                    </option>
                    @endforeach
                </select>
