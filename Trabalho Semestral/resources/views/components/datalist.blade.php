        ...
        <tbody>
            @foreach ($dados as $item)
                <tr>
                    ...
                    @foreach ($header as $hide)
                        @if($cont)
                            ...
                            @elseif($hide == 'cursos')
                                <td class="d-none d-md-table-cell">{{ $item->curso->nome }}</td>
                            @elseif($hide == 'genero')
                                <td class="d-none d-md-table-cell">{{ $item->genero->nome }}</td>
                            @else
                    ...
                    <td>
                        @if($acao[0] == true)
                            @if(UserPermissions::isAuthorized($crud.'.edit'))
                        ...
                        @endif
                        @if($acao[1] == true)
                            @if(UserPermissions::isAuthorized($crud.'.show'))
                                @if($crud != 'alunos')
                        ...
                        @endif
                        @if($acao[2] == true)
                            @if(UserPermissions::isAuthorized($crud.'.destroy'))
                        ...
                        @endif
                        @if($acao[3] == true)
                            @if(UserPermissions::isAuthorized('emprestimo.show'))
                            <a href="{{ route('alunos.show', $item['id'])}}" class="btn btn-warning btn-block align-content-center">
