    ...
    <div class="row">
        <div class="col" >
            <div class="form-floating mb-3">
                <input 
                    type="text" 
                    class="form-control @if($errors->has('crmv')) is-invalid @endif" 
                    name="crmv" 
                    placeholder="CRMV"
                    value="{{$dados['crmv']}}"
                />
        ...
        <div class="col" >
            <div class="input-group mb-3">
                ...
                <select 
                    name="especialidade"
                    class="form-select"
                    class="form-control @if($errors->has('especialidade')) is-invalid @endif" 
                >
                    <option value="" disabled selected>Selecione uma Especialidade</option>
                    @foreach ($especialidades as $item)
                        <option value="{{$item->id}}" @if($item->id == $dados->especialidade_id)  selected="true" @endif>
                            {{ $item->nome }}
                        </option>
                    @endforeach
                </select>
